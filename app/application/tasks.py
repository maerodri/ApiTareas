...
async def update_task(task_id: int, task_in: TaskUpdate, db: AsyncSession = Depends(get_db)):
    try:
        return await repositories.update_task(db, task_id, task_in.dict(exclude_unset=True))
    except NoResultFound:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")

@router.patch("/{task_id}/status", response_model=schemas.TaskOut)
async def change_task_status(task_id: int, status_in: TaskStatusUpdate, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_tasks_by_list(
    ...
    return await repositories.get_tasks_by_list_with_stats(db, list_id, status, priority)
